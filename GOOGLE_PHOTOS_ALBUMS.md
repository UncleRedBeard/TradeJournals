# Google Photos Album Inventory

Source: Google Photos shared albums intentionally added to TradeJournals.

Google Photos is handled differently from Flickr. Shared album pages are not
treated as a stable public metadata API. Durable image-level evidence should
come from one of these sources:

- a project JSON manifest
- a local Google Photos or Google Takeout export
- a future authenticated Google Photos importer

## Summary

- Albums tracked: 2
- Image-level representative picks completed: 0
- Albums pending representative image review: 2

## Album List

1. [Home Reno - Studio](https://photos.app.goo.gl/Tj2NRPeVUooFLAzD9)
   - Status: source recorded; representative image review pending
   - Section: residence
   - Journal:
     `01_the_residence_1894/trade_journals/ballet_barre_studio_restoration.md`
2. [Pottery](https://photos.app.goo.gl/eoqyVku1HMKUgy1W6)
   - Status: secondary mirror; primary evidence now maintained in Flickr
   - Section: materials and alchemy
   - Journal:
     `04_materials_and_alchemy/trade_journals/returning_to_clay_lockerbie_model_k.md`

## Album Details

<a id="album-af1qippool3ge7t"></a>

### 1. Home Reno - Studio

- Shared album URL: [Home Reno - Studio](https://photos.app.goo.gl/Tj2NRPeVUooFLAzD9)
- Canonical album URL: [Home Reno - Studio](https://photos.google.com/share/AF1QipPool3gE7T-_DVHyOZm3jEVm5TSnoL4P5kDdXAhzk-ja22Gbw3rsYOi8cn8EwffLA?key=S1hjd2FTdE9uTjMzX2NHNHpvYUZiQjA3YmpONTFR)
- Platform: Google Photos
- Visibility: shared URL verified by command-line fetch
- Photos: 204, confirmed by embedded Google Photos album metadata
- TradeJournals status: source recorded in dedicated residence journal
- Section: residence
- Existing journal: `01_the_residence_1894/trade_journals/ballet_barre_studio_restoration.md`
- Album date span: 2026-03-16 to 2026-05-31, from embedded Google Photos album metadata
- Next action: review representative images for ceiling plank replacement,
  floor refinishing and protection, wall mudding and sanding, ceiling fan
  wiring/mounting, exposed shiplap, and crown molding/coping.

<a id="album-pottery-eoqyvku1"></a>

### 2. Pottery

- Shared album URL: [Pottery](https://photos.app.goo.gl/eoqyVku1HMKUgy1W6)
- Platform: Google Photos
- Visibility: shared album reviewed in an authenticated browser session
- Photos: 30 at the latest review on 2026-08-03
- TradeJournals status: secondary source recorded in dedicated materials and
  alchemy journal
- Section: materials and alchemy
- Existing journal:
  `04_materials_and_alchemy/trade_journals/returning_to_clay_lockerbie_model_k.md`
- Album role: secondary capture and archive mirror for the renewed pottery
  practice
- Primary evidence source:
  [Flickr Pottery album](https://www.flickr.com/photos/boocher/albums/72177720334921846/)
- Cross-platform verification: all 30 Google Photos images are represented in
  Flickr; after the historical wood-fired upload, the primary Flickr album
  contains 68 additional images, for 98 unique public photos on 2026-08-04
- Clay-body evidence: porch vessels are Cinco Blanco; the original seven
  images are the only Cinco Rojo-only group; in two-plank worktable images,
  foreground vessels are Balcones Dark and rear-plank vessels are Cinco Rojo
- Next action: use Flickr photo IDs for clay-body comparisons, drying, bisque,
  glaze, and fired-result evidence.
